...
export async function apiFetch(path, options = {}) {
  ...
  // Global auto-logout hook. Any 401 anywhere in the app should tear
  // down the authenticated session — otherwise the user is stuck in a
  // half-authenticated state where the shell renders but every API
  // call silently fails (e.g. an empty shopping list that isn't really
  // empty). app/page.js listens for this event and calls its own
  // handleSessionExpired().
  //
  // We fire on the browser only (no-op during SSR) and DO NOT gate on
  // caller opt-in — every 4-5x call site would otherwise need its own
  // 401 branch. Emitting an event is a no-op on pages that don't
  // listen, so this is safe.
  if (code === 'SESSION_EXPIRED' && typeof window !== 'undefined') {
    try {
      window.dispatchEvent(new CustomEvent('forkcast:session-expired', {
        detail: { path, status: response.status },
      }));
    } catch {
      /* CustomEvent unsupported → the world has bigger problems */
    }
  }
